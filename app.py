...
# 頁面設定
st.set_page_config(page_title="股票分析系統", layout="wide")
...
if analyze_btn and queries:
    ...
    for i, query in enumerate(queries):
        with tabs[i]:
            ...
            # 得分與評論
            rating = "🚀 強勢標的" if score >= 75 else "⚖️ 穩健標的" if score >= 50 else "⚠️ 看就好" if score >= 25 else "⚠️ 不要碰"
            st.markdown(f"### 📊 綜合診斷：{score} 分 | {rating}")
            st.write(f"💬 分析評論：{'多方標的，適合順勢操作。' if score >= 75 else '格局穩定，建議分批佈局。' if score >= 50 else '訊號疲弱，建議保守觀望。' if score >= 25 else ''弱勢標的，不要看了。' if score >= 80 else '}")
            ...
            with c3:
                st.markdown(get_metric_html("止損價位", sl_p, "green"), unsafe_allow_html=True)
            with c4:
                st.markdown(get_metric_html("停利價位", tp_p, "red"), unsafe_allow_html=True)
            st.markdown("---")
